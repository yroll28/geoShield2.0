...
async function loadRequestsFromServer(){
  ...
  try{
    ...
    if(statusEl){
      if(data.storage==="local-file"&&!location.hostname.includes("localhost")&&!location.hostname.startsWith("127.")){
        statusEl.textContent="⚠ This deployment is using local-file storage, which does NOT reliably persist on Vercel/serverless hosting. Requests may disappear or fail to appear on other devices. Connect a Redis database in the Vercel Storage tab — see CHAT_SETUP.md.";
      }else{
