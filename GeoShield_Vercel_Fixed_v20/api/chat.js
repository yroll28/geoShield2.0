 ...
 * serverless invocations); falls back to a local data/chat-threads.json file
 * for plain Node hosting (e.g. `node server.js`). See api/_store.js for details.
 * Required for admin access: CHAT_ADMIN_KEY.
 * Optional email notifications: RESEND_API_KEY + MAIL_FROM + CHAT_ADMIN_EMAIL.
 */
const crypto = require('crypto');
const store = require('./_store.js');
const chatThreads = store.dataset('geoshield:chat-threads', 'chat-threads.json', {});
...
module.exports = async function handler(req,res){
  ...
  if(req.method==='GET'){
    ...
    const threadsObj=await chatThreads.read();
    if(admin && !requestId){
      const threads=Object.values(threadsObj).sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt))).map(safeThread);
      return json(res,200,{ok:true,threads});
    }
    const thread=threadsObj[requestId];
  ...
  if(action==='create'){
    ...
    const threadsObj=await chatThreads.read();
    let thread=threadsObj[requestId];
    if(thread){
    ...
    threadsObj[requestId]=thread;
    await chatThreads.write(threadsObj);
  ...
  if(action==='message'){
    ...
    thread.messages.push(msg);thread.updatedAt=msg.createdAt;
    await chatThreads.write(threadsObj);
