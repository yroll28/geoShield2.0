...
function req(){try{return JSON.parse(localStorage.getItem("geoRequests")||"[]")}catch(e){return[]}}
...
async function sendAutomaticCompletionEmail(id){
  ...
  localStorage.setItem("geoRequests",JSON.stringify(r));
  requests();
  try{
    ...
    if(saved){
      ...
      localStorage.setItem("geoRequests",JSON.stringify(fresh));
    }
    requests();
...
function openGmailCompletion(item){
  ...
  localStorage.setItem("geoRequests",JSON.stringify(req()));
  if(!win){
